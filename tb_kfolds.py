...
from const import RESDIR, PATIENTS, W, FS, SEED


parser = argparse.ArgumentParser(
    description='Processes the MIT-CHB dataset to create training/test cross-validation splits.')

parser.add_argument('--sph', required=True, default=0,
                    help='Seizure prediction horizon (minutes). \
                    Default: 0')

parser.add_argument('--sop', required=True, default=10,
...
for patient_idx in range(len(PATIENTS)):
    ...
    f = open(os.path.join(outSubDir, outfile), 'wb')
    pickle.dump(X, f)
    f.close()
